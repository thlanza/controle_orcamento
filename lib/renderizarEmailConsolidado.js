...
export function renderizarEmailConsolidado({ titulo, consolidado }) {
  ...
  const linhasUltimosGastos = (consolidado.ultimosGastos || [])
  .map((gasto) => {
    return `
      <tr>
        ...
        <td>${gasto.moeda}</td>
        <td style="text-align: right;">${formatarValorDecimalParaReais(gasto.valor)}</td>
      </tr>
    `;
  })
  .join("");

  const corpoUltimosGastos = linhasUltimosGastos ||
  `
  <tr>
